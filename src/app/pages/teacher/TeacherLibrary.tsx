...
export function TeacherLibrary() {
  ...
  const fetchBooks = async () => {
    const { data, error: fetchError } = await supabase
      .from("library_books")
      .select("id, title, author, category, pdf_url, uploaded_by_user_id, uploaded_by_role, uploaded_by_name, created_at")
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      setActorRole(teacher.role || "teacher");
      await fetchBooks();
      setLoading(false);
    };
  ...
  const handleUploadBook = async (e: React.FormEvent) => {
    ...
    const filePath = `teacher-${teacherId}/${Date.now()}-${safeTitle}.pdf`;
    ...
    const { error: insertError } = await supabase.from("library_books").insert({
      ...
      uploaded_by_name: user?.fullName || "Teacher",
    });
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-1">Library PDFs</h1>
        <p className="text-muted-foreground">Upload books so students can read them anytime</p>
      </div>
      ...
      <div className="space-y-3">
        {books.map((book) => (
          <div key={book.id} className="bg-card border border-border rounded-lg p-4 flex items-center justify-between gap-4">
            ...
            {(actorRole === "admin" || book.uploaded_by_user_id === teacherId) ? (
              <button
                type="button"
