...
export function TeacherNotices() {
  ...
  const [actorRole, setActorRole] = useState<string>("teacher");
  ...
  const fetchNotices = async () => {
    const { data, error: fetchError } = await supabase
      .from("notices")
      .select("id, title, content, category, date, author, pinned, urgent, posted_by_user_id, posted_by_role, posted_by_name")
  ...
  useEffect(() => {
    const bootstrap = async () => {
      ...
      if (actorError || !actor?.id) {
        setError("Could not resolve your teacher account.");
        setLoading(false);
        return;
      }

      setTeacherUserId(actor.id);
      setTeacherName(actor.name || user.fullName || "Teacher");
      setActorRole(actor.role || "teacher");
  ...
  const handleCreateNotice = async (e: React.FormEvent) => {
    ...
    const payload = {
      ...
      pinned: form.pinned,
      urgent: form.urgent,
      posted_by_user_id: teacherUserId,
      posted_by_role: "teacher",
      posted_by_name: teacherName,
    };
    ...
    setForm({ title: "", content: "", category: "Academics", pinned: false, urgent: false });
  };
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="mb-8 flex items-center justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold text-foreground mb-1">Manage Notices</h1>
          <p className="text-muted-foreground">Post latest updates for all students</p>
        </div>
      </div>
            ...
            {notices.map((notice) => (
              <tr key={notice.id} className="border-b border-border">
                ...
                <td className="px-6 py-3 text-right">
                  {(actorRole === "admin" || notice.posted_by_user_id === teacherUserId) ? (
                    <button
                      type="button"
                    ...
                    <span className="text-xs text-muted-foreground">Only owner can delete</span>
                  )}
                </td>
              </tr>
            ))}
